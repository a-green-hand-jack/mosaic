...
This follow-up extends the minimum Protenix-backed smoke with two small runs:
...
The current soft-cone solution matches normalized weighted in these small Protenix runs, which suggests that gradient normalization alone is a strong baseline and must remain in the main comparison.
