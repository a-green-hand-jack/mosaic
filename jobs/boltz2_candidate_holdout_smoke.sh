...
args=(
  --candidates-csv
  "${CANDIDATES_CSV:-docs/results/phase0_protenix_cem_2dd3965_20260507T120914Z_candidates.csv}"
  ...
  --balance-by "${BALANCE_BY:-none}"
  --recycling-steps "${RECYCLING_STEPS:-1}"
  --sampling-steps "${SAMPLING_STEPS:-1}"
  --run-label "${RUN_LABEL:-cem2dd3965_top5}"
)

if [[ -n "${METHOD_IDS:-}" ]]; then
  args+=(--method-ids "${METHOD_IDS}")
fi

uv run python -u scripts/score_boltz2_candidate_holdout.py "${args[@]}"
